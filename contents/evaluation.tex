...
本文在客观指标层面采用四项度量：Fréchet Gesture Distance(FGD)、语义相关动作召回率(Semantic Relevance Gesture Recall, SRGR)、节奏对齐度(Beat Alignment, BA)以及L1范数(L1DIV)。
...
分别为第$l$层的可学习权重矩阵与偏置项，$\sigma(\cdot)$为非线性激活函数。
上述局部邻域聚合过程可形式化表示为：
\begin{equation}
\mathbf{h}_i^{(l+1)} =
\sigma\left(
...
SRGR 指标（Semantic Relevance Gesture Recall）\cite{beatcamn}
...
反映模型在时序同步性（temporal synchronization）方面的性能。
与SRGR基于阈值的逐帧数值匹配不同，BA更关注语音节拍事件与手势关键动作事件之间的时间对齐精度。

\paragraph{定义与原理}
设语音节拍集合为$\mathcal{B}_s = \{t^s_k\}$，
通过检测语音短时能量或梅尔倒谱系数（Mel-Frequency Cepstral Coefficients, MFCC）的局部峰值得到；
...
因此$\mathrm{BA}$越接近1，表明手势动作越能准确响应语音重读节拍。
