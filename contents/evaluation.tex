...
\caption{实验对比模型的输入输出模态对比。$\ast$ 表示模型结构未显式输入说话人ID，采用每位说话人独立训练的设置，说话人ID通过模型参数指定。}
...
%明确说明过如果观察到不自然的突然抖动，则降低真实感的评分。通过测试发现，电脑用户基本会察觉到DiffSHEG的抖动，而VR用户除非正在注视，不一定能察觉到DiffSEHG的抖动。可能是因为以一定距离观察整个电脑屏幕内容，和贴近眼睛观察VR世界，会有集中力范围的区别。这个因素一定程度上影响了本次的研究结果。

%不过，也观察到DiffSEHG更擅长在每个单词的短间隔上做出丰富细腻的动作，推测语义识别帮助了它可以清晰地识别每个单词的分界线。而FaceCapGes很难有每个单词级别的起伏动作，除非说话人做出了较为明显的声调变化或情绪变化。
...
%Axis-Angle在角度表达上存在循环，可能会学到值的跳跃的情况。而我们的模型Rot6d在所有地方上平滑，所以整个测试中没有收到任何抖动的反馈。
相比之下，
...
全程不依赖未来帧信息，完全匹配实时交互场景的数据流特性。
为精准模拟真实应用中的实时输入流场景，性能测试基于 BEAT 数据集的测试集展开，
采用 Batch=1 的配置关闭批处理机制，确保每帧数据均独立输入模型进行推理，还原逐帧处理的实际运行状态。
测试过程中，我们将测试集中 93015 帧的多模态输入数据完整传入模型，记录从首帧输入到末帧输出的总推理耗时，
通过总时长与测试帧数的比值计算平均单帧处理时间。
测试结果如表～\ref {tab3} 所示，在 RTX4090 硬件环境下，模型平均单帧处理时间仅为 6.07 毫秒，
具备优异的实时响应能力。
