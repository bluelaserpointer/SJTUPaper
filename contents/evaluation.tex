...
确保所有被试对评分维度理解一致：
\begin{itemize}
    \item 真实性：整体动作是否自然流畅，是否存在明显的违和感，如朝向异常或突然抖动；
    \item 同步性：手势动作与语调、语音节奏是否协调一致；
    \item 多样性：手势是否丰富多变，避免长时间静止或重复单一动作。
\end{itemize}
...
\begin{itemize}
    \item 桌面端环境：参与者通过个人电脑或实验室台式机进行评估，
实验界面采用三窗口并行布局，参与者可同时观察左、中、右三个区域的虚拟人动作，
聚焦于整体动作风格、节奏同步性的直观对比，注意力分布于整个屏幕的动作全局表现。
    \item VR端环境：基于 PICO VR 设备\cite{pico4}搭建沉浸式评估场景，参与者佩戴 VR 头显后进入虚拟观测空间，
...
本文在客观评估中采用四项度量：手势分布相似度（Fréchet Gesture Distance，FGD）\cite{speech_gesture_generation}、语义相关动作召回率（Semantic Relevance Gesture Recall，SRGR）\cite{beatcamn}、节奏对齐度（Beat Alignment，BA）\cite{beatcamn, beatalign}以及L1范数（L1DIV）。
这些指标分别对应生成动作在分布一致性、语音同步性与变化丰富性等不同维度，
共同构成对模型质量的综合评估体系。

\subsection{手势分布相似度（Fréchet Gesture Distance）}
\label{subsec:fgd}

手势分布相似度（Fréchet Gesture Distance，FGD）\cite{speech_gesture_generation}用于衡量生成手势分布与真实手势分布之间的统计距离，
...
\subsection{语义相关动作召回率（Semantic Relevance Gesture Recall）}
\label{subsec:srgr}

语义相关动作召回率（Semantic Relevance Gesture Recall，SRGR）\cite{beatcamn}
...
\subsection{节奏对齐度（Beat Alignment）}
\label{subsec:ba}

节奏对齐度（Beat Alignment，BA）指标用于衡量语音节拍事件与手势关键动作事件在时间轴上的对齐程度，
...
L1范数（L1DIV）\cite{beatcamn}用于衡量模型生成手势序列的多样性，
