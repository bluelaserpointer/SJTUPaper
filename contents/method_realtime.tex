\chapter{面向在线实时手势生成的闭环自回归训练框架}
\label{chap:training_strategy}

第\ref{chap:model_architecture}章给出了满足因果约束的单步动作预测器，用于在给定当前输入与有限历史动作条件下预测下一帧动作。但在实际在线生成中，模型需要在闭环自回归条件下连续输出多帧序列，预测结果将被不断写回并作为后续步骤的历史条件。在这一设置下，早期预测误差可能逐步累积并引发动作漂移，单步监督难以直接约束这种序列级不稳定性。

针对这一问题，训练自回归模型存在两种基本策略：
teacher forcing以真实历史帧作为训练阶段的历史条件，实现简单直接；free-running则在训练阶段同样以模型自身的预测输出作为历史条件，使训练与推理的输入分布保持一致，从而在理论上缓解训练与推理之间的分布偏移（exposure bias）。两种策略在推理阶段均依赖模型自身历史输出进行自回归生成，其差异仅体现在训练阶段历史条件的来源。

本章在单步预测器基础上，构建两种策略共同依赖的闭环展开训练框架：
通过滑动窗口逐帧展开与历史写回机制构造连续生成区间，并在生成区间上定义片段级监督损失与对抗训练目标。
本章重点描述上述框架的实现，其中两种训练策略的差异将在第~\ref{subsec:streaming_buffer}~节中明确指出。
关于两种策略在在线实时生成任务中的实际性能对比，将在第~\ref{subsec:ablation}~节消融实验中进行系统评估。
...
本方法参考CaMN\cite{beatcamn}采用带重叠的滑动切割方式从长序列中提取训练片段。
...
由式\eqref{eq:segment_stride}可得到一组片段之间相互重叠的训练片段$\{\bm{V}_k^B, \bm{Z}^{in}_k\}$：

\begin{align}
\bm{V}_k^B &= (\bm{v}_{s_k}^B, \bm{v}_{s_k+1}^B, \ldots, \bm{v}_{s_k+L-1}^B)\\
\bm{Z}^{in}_k &= \left(\{ \bm{z}_{s_k:t}^A \}_{t=s_k}^{s_k+L-1},\ \{ \bm{z}_{s_k:t}^F \}_{t=s_k}^{s_k+L-1},\ \{ \bm{z}_{s_k:t}^H \}_{t=s_k}^{s_k+L-1}\right)
...
为便于后续描述，本方法将第$k$个训练片段按时间划分为前置动作帧区间与生成区间。
前置动作帧定义为
...
为形式化描述上述过程，本方法在片段内部采用滑动窗口方式对单步预测器进行展开。
...
本方法维护一个历史动作缓存$\mathcal{H}$，用于存储当前可用的最近$N$帧动作：
...
在时间步$t$，本方法将缓存中的历史动作与当前可观测多模态输入共同组织为长度为$N{+}1$的因果上下文窗口，
...
在片段起点$s_k$，首先进行预热阶段：将前置动作帧$\bm{V}_k^{pre}$写入历史动作缓存$\mathcal{H}$，并同步读取对应的多模态输入特征。
...
上述历史缓存更新方式对应free-running训练策略：
在生成阶段，历史条件完全由模型自身预测结果填充，与推理阶段的实际条件保持一致。
作为对照，teacher forcing策略在训练阶段将式\eqref{eq:history_update_stream}中的$\hat{\bm{v}}_{t}^{B}$​替换为对应时刻的真实动作帧$\bm{v}_{t}^{B}$​，即历史缓存始终由真实序列维护，而非模型预测结果。两种策略在推理阶段的行为完全相同，差异仅在于训练时历史条件的来源。

在训练时，本方法将每个长度为$L$的片段按时间顺序拆解为逐帧输入流：模型在每个时间步接收当前帧的多模态输入特征，并通过上述历史缓存机制构造$\bm{Z}_t^{fuse}$以完成预测。
在生成阶段的$M$个时间步上，本方法收集逐帧输出$\hat{\bm{v}}_{t}^{B}$并沿时间维拼接，得到与生成区间对齐的连续预测序列$\hat{\bm{V}}_k^{gen}$，
...
在片段内展开过程中，本方法在每一步预测后保留更新得到的$(\bm{h}_t^{T},\bm{c}_t^{T})$与$(\bm{h}_t^{U},\bm{c}_t^{U})$，
...
因此$\bm{V}_k^{gen},\hat{\bm{V}}_k^{gen}\in\mathbb{R}^{M\times d}$。
...
为同时保证空间重构精度与时间连续性，本方法采用基于Huber误差的重构损失形式，并分别作用于姿态、速度与加速度信号。
...
为约束时间平滑性，本方法进一步对生成区间的速度与加速度信号施加同类误差约束。
...
具体而言，对于每个训练批次，本方法首先固定生成器参数$\theta$，最小化式\eqref{eq:disc_loss_bce}更新判别器参数；
...
具体而言，本方法沿用基线模型CaMN的重叠片段切割策略，并引入长度为$N$的前置动作帧用于初始化历史动作条件；
...
上述框架同时构成teacher forcing与free-running两种训练策略的共同实现基础，
两者仅在训练阶段历史条件的来源上存在差异。

关于两种策略在在线实时生成任务中的性能对比，将在第五章消融实验（第~\ref{subsec:ablation}~节）中进行系统评估。
实验结果表明，free-running策略能够有效缓解exposure bias，在手势分布相似度与多样性指标上均优于teacher forcing。