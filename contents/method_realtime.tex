...
针对这一问题，一个自然的训练选择是teacher forcing——以真实历史帧作为条件进行训练。
然而，teacher forcing在训练阶段屏蔽了模型自身预测误差的影响，与推理时的实际条件存在分布偏移，即exposure bias问题。
因此，本章在单步预测器基础上构建闭环展开的训练形式，并在生成区间上施加序列级训练目标，以提升连续生成的稳定性。

本章首先介绍基于滑动窗口展开的闭环自回归训练框架，作为两种策略共同依赖的基础结构；随后在此框架下对比teacher forcing与free-running的训练效果，验证free-running对缓解exposure bias的有效性。
