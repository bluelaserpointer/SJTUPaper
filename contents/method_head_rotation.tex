...
第\ref{sec:system}节提到，本文设计的框架中，系统配置层支持按说话人选择模型参数，但在本文任务设定中，说话人ID不参与网络输入。
这是因为，在实时交互的单次运行过程中，用户身份通常保持不变，系统无需在帧级推理过程中频繁切换说话风格。
因此，模型无需显式感知并分离属于说话人风格的成分；相反，将风格选择上移至系统层即可满足使用需求。
基于上述考虑，FaceCapGes 不引入说话人ID输入模态，采用针对每个说话人独立训练并部署模型参数的方式，
从而减少网络输入与分支结构带来的建模负担与耦合复杂度，同时保持实时推理流程的简洁性。
