...
随后，融合输入序列$\bm{Z}_t^{fuse}$分别送入躯干与上肢两路单向LSTM解码器，得到窗口内的输出序列：
\begin{equation}
...
关于循环状态的传递方式、滑动窗口展开策略以及片段级优化目标，将在第\ref{sec:one_step_lstm}章与第\ref{sec:sliding_window_rollout}章中进一步说明。
...
同时，本文仅使用头部旋转信息进行建模，并给出了适用于不同数据来源（面部捕捉系统与数据集骨架数据）的统一特征获取与表示方式，从而提升跨场景泛化能力与时序连续性。
