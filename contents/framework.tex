...
本章围绕本文提出的多模态在线实时手势生成系统，给出从输入信号获取到数据表示与端到端建模的整体框架分析。
...
与依赖穿戴式传感器、多机位相机阵列或专业动捕棚的方案不同，
本文将输入侧约束为通用单设备条件，即系统仅依赖麦克风与前向摄像头即可完成所需信号的采集与解析。
在该设定下，系统能够稳定获得以下输入信息：

\begin{itemize}
    \item \textbf{语音信号（Audio）}：
    通过设备麦克风实时采集，为后续的语音特征提取提供原始波形输入。
    \item \textbf{面部参数（Expression Parameters）与头部姿态（Head Pose）}：
...
以及如何在统一表示与预处理之后构建端到端在线实时手势生成架构。

\section{训练数据来源}
近年来的语音驱动手势生成研究通常以公开数据集为基础展开。
其研究工作常见的流程是：首先确定所采用的数据集，并以其提供的动作捕捉与表情采集格式作为统一表示的基础。
在这一前提下，骨架拓扑、关节驱动范围、面部表情参数空间以及各模态的同步方式，均由数据集定义并约束模型的输入输出形式。

因此，本节首先对本文所采用的训练数据集BEAT\cite{beatcamn}进行介绍与分析。
BEAT 是一个面向虚拟角色驱动的多模态动作数据集，其核心优势在于同时提供了语音、上半身动作捕捉与面部表情捕捉等信息，
并保证了模态间的时间同步关系。

我们将从数据集的多模态构成出发，说明其骨架拓扑结构与面部表情表示方式。
...
从而定义出一棵有根树结构。
系统在每一帧$t$输出该骨架上所有关节的局部旋转$\mathbf{r}^{(j)}_t$，从而构成虚拟人身体姿态的完整描述，形式化定义为：
...
BEAT 数据集的标准骨架结构如图~\ref{fig_beatbones} 所示，共包含人体中的 47 个关节节点，
...
在BEAT数据集中，面部参数由Apple ARKit\cite{ARKitDocumentation}定义的多个形变基组合表示，
在计算机图形与动画领域中，这个形变基系统被称为BlendShape。

其思想来源于Facial Action Coding System（FACS）\cite{EkmanFriesenFACS1978}，
将复杂表情分解为若干可组合的基本动作单元（Action Units, AU），
从而为将面部表情描述为一组可控参数。
BlendShape中，每个基形由一个标量权重控制局部表情变化。
通过调节多个基形的权重并进行组合，即可生成丰富的复杂表情。
BlendShape没有固定的规范，动捕软件、三维人物模型可自定义自己的形变基组合，
但通常只有基于相同规范的系统间才可以复用面部参数。

以闭眼为例，FACS将闭上双眼的动作设为一个动作单元，如图~\ref{fig:au_sample}所示。
\begin{figure}[h!t]
\centering
...
在BEAT数据集使用的Apple ARkit定义的BlendShape标准中，上述动作对应两种基形：闭左眼与闭右眼。
...
系统在每一帧$t$输出该所有的形变基权重$\mathbf{w}^{(k)}_t\in\![0,1]$，从而构成虚拟人面部的完整描述，形式化定义为：
...
以及近年来在神经网络回归任务中广泛使用的连续旋转表示，如$6$维旋转表示（Rot6D）。
本文主要比较以下几种形式在工程系统中的适用性：

\begin{itemize}
    \item 欧拉角： 
    参数维度低且直观，没有冗余信息，适合存储。
    但存在万向节锁（gimbal lock）问题，且角度空间不连续，
    在训练使用容易导致预测输出在临界角附近出现跳变，从而破坏实时动画的平滑性。
    \item 轴角表示：
    可用三维向量描述旋转，但在接近零旋转时存在不稳定性，
    且角度周期性带来的不连续同样会对网络回归造成影响。
    \item 四元数：
    能避免万向节锁并具有较好的数值性质，但其单位范数约束使得网络直接回归时需要额外规范化步骤；
    同时四元数具有符号二义性（$\mathbf{q}$与$-\mathbf{q}$表示同一旋转），可能导致学习目标不一致。
    \item 旋转矩阵：
    表达完整且无二义性，但需要满足正交约束与行列式为$1$的条件（$\mathrm{SO}(3)$群约束），
    直接回归时需额外投影或正交化处理，否则难以保证输出有效性。
\end{itemize}

综合考虑网络学习的稳定性、输出连续性与工程部署便利性，
本文采用Rot6D作为关节局部旋转的统一表示形式。
Rot6D通过取旋转矩阵的前两列向量（记为$\mathbf{a},\mathbf{b}\in\mathbb{R}^3$）
组成一个$6$维向量，并通过Gram-Schmidt正交化恢复完整的旋转矩阵，
从而避免了显式的正交约束回归问题。
具体而言，设模型输出的Rot6D为$\mathbf{r}=[\mathbf{a};\mathbf{b}]\in\mathbb{R}^6$，
...
根据式\eqref{eq:body_pose_vector}，训练中的身体姿态表示为：$\bm{v}^{B}_t \in \mathbb{R}^{6 \times 47}.$
...
随后，本章基于 BEAT 数据集说明训练数据的模态构成与同步关系，并分别给出骨架拓扑与面部 BlendShape 参数化表示；同时补充头部姿态的表示与其与 ARKit 输出接口的对应关系。 
