\chapter{在线实时手势生成系统的需求分析、数据表示与端到端框架}
...
\section{训练数据来源}
为在训练阶段引入面部模态监督，本文采用 Body-Expression-Audio-Text（BEAT） 数据集~\cite{beatcamn}。
...
在确定数据来源与模态表示后，还需选择适合网络学习的旋转参数化方式。
...
因此，需要将BEAT数据集的原始旋转表示（欧拉角）在预处理时进行转换。
...
此时，可以按如下方式恢复旋转矩阵 $\mathbf{R}\in\mathbb{R}^{3\times 3}$：
...
根据式 \eqref{eq:body_pose_vector}，训练集中第 $t$ 帧的身体姿态可表示为
$\bm{v}^{B}_t \in \mathbb{R}^{6 \times 47}$。

同样地，根据式 \eqref{eq:head_pose_vector}，训练集中第 $t$ 帧的头部姿态表示为
$\bm{v}^{H}_t \in \mathbb{R}^{6}$。

\section{在线实时手势生成系统整体框架}
\label{sec:system}
本节介绍系统的整体驱动流程及模块划分。如图~\ref{fig:system_architecture}所示，系统由四个层级构成：设备层、中间件层、手势生成模型层以及渲染驱动层。各层通过统一的多模态数据接口进行通信，实现从信号采集到虚拟人动作生成的实时端到端处理。

FaceCapGes模型位于手势生成模型层，承担多模态输入到上半身姿态输出的核心推理任务；输入采集模块与渲染模块分别负责信号获取与结果展示。

为实现实时数字人驱动框架，本文完成了手势生成模型层与渲染驱动层的实现，并构建了系统原型。FaceCapGes模型作为系统的核心推理模块，在实时约束条件下从多模态输入中预测当前帧的上半身骨骼姿态。

由于本文所使用的数据集 BEAT\cite{beatcamn} 依赖基于 iOS 设备的面部捕捉工具 ARKit\cite{ARKitDocumentation}，为便于说明，以下以 iOS 设备为例介绍各层实现细节。在实际部署至其他设备（例如个人计算机）时，需要采用不依赖深度图的面部捕捉方案，并准备与其参数形式一致的训练数据。
...
这两层位于系统架构的输入端，负责多模态信号的采集与预处理。

（1）设备层：
设备层负责采集语音与视觉模态信号。语音信号由麦克风实时录制；视觉信号由前向摄像头获取 RGB 图像与深度图数据，并作为 ARKit 面部追踪模块的输入数据源。

（2）中间件层：
中间件层对设备层传入的音频与视觉信号进行处理。其中，音频流在中间件层中被送入特征提取模块以生成时间序列特征。模型训练阶段使用 Librosa 库\cite{librosa}离线提取 Mel 频谱、短时能量与基频 $F_0$ 等声学特征，以保证特征精度与一致性。
在实际部署为实时系统时，可采用与训练阶段等价的在线特征提取模块，对连续音频流进行分帧计算并生成对应声学特征，从而实现低延迟推理。该模块在系统架构中作为音频预处理单元存在，但其具体实现方式不影响本文模型结构与推理机制。

视觉信号通过 Apple 提供的 ARKit 框架\cite{ARKitDocumentation}，将原始图像流与深度图转换为结构化特征。ARKit 输出两类主要数据：

面部表情特征：
ARKit 提供 52 维 BlendShape 系数向量，用于描述面部肌肉群的局部形变状态。该特征能够间接刻画用户的表情与口型变化，并以帧级形式同步输出。

头部姿态特征：
ARKit 在 ARFaceAnchor 中提供一个齐次变换矩阵 $\mathbf{T}\in\mathbb{R}^{4\times4}$，
...
其中 $\mathbf{R}$ 表示旋转矩阵，$\mathbf{t}$ 表示平移向量。本研究提取矩阵中的旋转部分 $\mathbf{R}$，
并将其转换为 Rot6D~\cite{rot6d} 表示形式，以提升旋转空间的连续性与模型训练的稳定性。

经中间件处理后的多模态特征通过统一的数据接口传递至手势生成模型层，实现语音、表情与头部姿态信息的实时融合输入。

\subsection{手势生成模型层}

FaceCapGes 模块作为系统的核心推理单元，接收来自信号采集的三类输入特征：语音特征、面部BlendShape系数以及头部姿态参数，并在不依赖未来帧的条件下，逐帧预测用户当前时刻的上半身骨骼姿态。模型的内部结构如图~\ref{fig_architecture}~所示，其详细设计将在第~\ref{chap:model_architecture}~章展开介绍。

生成的骨骼姿态采用Rot6D\cite{rot6d}连续旋转表示形式，覆盖上半身47个关节的旋转参数。模型内部通过级联多模态编码结构提取时序相关特征，并利用单向长短期记忆网络（Long Short-Term Memory, LSTM）\cite{lstm}解码器完成时间依赖建模，从而在保持实时性的同时，生成与语音节奏、表情变化及头部朝向高度一致的自然手势。

FaceCapGes输出的姿态数据通过统一接口传递至渲染驱动层，与实时面部捕捉信号共同驱动虚拟角色的整体动作。由于模型仅依赖当前与历史帧输入，可与输入层以固定帧率并行运行，实现端到端的低延迟推理。
...
本文在Unity\cite{unity}渲染阶段使用Mechanim提供的人形骨架映射机制\cite{unitymechanim}，
